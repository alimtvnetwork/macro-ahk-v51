...
> - Only the **bare repo name** is rewritten (e.g. `macro-ahk-v49` → `macro-ahk-v49`).
...
… (URL-safety) skipped 3 matches inside URLs:
  readme.md:42  https://github.com/alimtvnetwork/macro-ahk-v49
  …
...
| ID | Criterion |
|----|-----------|
| AC-RR-001 | Running `scripts/repo-rename.sh` with no args in a repo whose remote is `…/macro-ahk-v49` shows a dry-run with `From: macro-ahk-v49` / `To: macro-ahk-v49`. |
...
| AC-RR-003 | URL strings (`https://github.com/org/macro-ahk-v49`, `git@github.com:org/macro-ahk-v49.git`) are reported as URL skips and never rewritten. |
| AC-RR-004 | A line containing both `macro-ahk-v49` (bare) and `https://github.com/org/macro-ahk-v49` is partially rewritten — only the bare token changes. |
| AC-RR-005 | `macro-ahk-v210` is **not** matched when renaming `macro-ahk-v49` (whole-token rule). |
...
| `--commit` | Stage and commit the rewrite as a single conventional commit (`chore: rename macro-ahk-v49 → macro-ahk-v49`). |
