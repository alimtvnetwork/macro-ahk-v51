# 07 — Proofread / Rewrite

Captured 2026-05-22. Trigger phrases: `proofread`, `proof read`,
`rewrite`, `rewrite next`, `next` (in proofread mode).

**Activation:** When the user invokes any trigger above, switch into
proofread-only mode — DO NOT ACT on the content. Only rewrite it per
the rules below.

**Related commands** (also handled here):
- `revise prompt` / `revise memory` / `read memory` → re-read ALL files
  under `.lovable/prompts/` plus `.lovable/prompts.md` index, plus
  memory, with strict attention to every file (no skipping).
...
Also, remember "revise prompt" or "revise memory" or "read memory" meaning reading ALL the prompt files (`.lovable/prompts/*` — all files without confusion, strict attention) and index from lovable memory, and also save this as a command in `.lovable/prompts.md`.
...
In your prompts, always ask "if you have any question and confusion, feel free to ask, and if you are creating tasks for creating multiple tasks, and if it is bigger ones, then do it in a way so that if we say next, you do those remaining tasks. Do you understand? Always add this part at the end of the writing inside the code block. Do you understand? Can you please do that?" — first proofread and this part at the end always.

All the prompts and conversation that I request or ask or ask you to do — you create a folder in the root `/conversation/xx-feature/xx-title-of-conv.md` and `/conversation/index.md` should contain the conversation indexing, and also mention or add these instructions to every proofread at the end with additional instructions, and also mention to write this same thing if a `next` command is given so that AI is reminded again and again.
...
15. If any designs or assets are given, put those to `/assets/xx-folder-name/xx-file-name.jpg` or `.png` or `.mp3` or anything else; keep the `xx` for sequence

Write these coding guidelines in the lovable memory (`.lovable/coding-guidelines.md`) — or update if it exists, properly for AI blind follow. If it does not exist, create it; if it exists, enhance it; and also mention the files to read explicitly from paths and the spec folder.
...
If I describe email flows or multi-step processes, document each step sequentially and in detail. Missing steps will break execution, so completeness is mandatory.
...
- Use PascalCase naming (per global rule above) — note: legacy "camelCase" mention in original is overridden by the Pascal Case rule
- Prefer ORM usage
...
> TO AI: Write the spec first in detail for this given verbatim and tasks, and also plan first in memory and in `plan.md`. Then start implementing as the user says `next` in each phase, and list the remaining tasks only if the task is very big and requires iterations.
>
> If you have any question and confusion, feel free to ask. If you are creating multiple tasks and they are bigger ones, do it in a way so that when we say `next`, you do those remaining tasks. Do you understand?
