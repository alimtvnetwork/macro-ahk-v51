...
| 19.5 | Step inspector UI for `UrlTabClick` editor | ✅ 2026-05-16 (`StepKindId.UrlTabClick = 9` added to step-library enum + seed; `StepEditorDialog` gains a structured form: URL pattern + dialect (Exact/Prefix/Glob/Regex), Mode (Open/Focus/OpenOrFocus), optional Selector + SelectorKind, TimeoutMs, DirectOpen+Url; payload serialised PascalCase mirroring `UrlTabClickParams`; replay-bridge throws precise reason for UrlTabClick; idempotency test bumped 7→8; 21/21 db tests + 239/240 step-library tests green) |
