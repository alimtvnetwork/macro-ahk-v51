...
| **0.8** — ESLint `id-denylist` rule | Root cause fixed 2026-06-04: global ESLint was scanning generated/cache outputs (`chrome-extension/`, `.cache/`, `test-results/`), then the `msg`/`ctx` global pass initially missed 107 legacy-debt paths and created 883 lint errors. Corrected 2026-06-04: generated artifacts ignored; `val` closed; `cb`/`obj`/`fn`/`el`/`msg`/`ctx` globally banned for new/cleaned files; all legacy debt explicitly quarantined and pinned by `scripts/__tests__/eslint-rules.test.mjs`. Next-2 pass 2026-06-04: graduated 4 clean quarantine entries and closed standalone `globalThis.console.error` bypass via logger shims + selector guard. Recent cleanup pass: graduated `src/lib/open-extension-options.ts` (`cb` → `callback`) plus already-clean `standalone-scripts/payment-banner-hider/src/globals.d.ts`, and pinned both as cleaned files in the ESLint-rule test. Current next-2 pass: graduated `standalone-scripts/macro-controller/src/gitsync/disconnect-repo.ts` (`msg` → `confirmMessage`) and `standalone-scripts/macro-controller/src/startup-persistence.ts` (`cb` → `callback`). Remaining work: retire the 258-entry explicit quarantine + 3 glob quarantines gradually. | Staged |
...
|---|------|--------|--------|---------|
| 1 | **Priority 0.8** — continue staged authored-source id-denylist quarantine retirement (`cb`, `obj`, `fn`, `el`, `msg`, `ctx`) | High | Medium — 8 entries graduated; 258 explicit quarantine entries + 3 glob quarantines remain | Effort budget |
