...
4. **Sibling-repo discovery** _(opt-in, discovery mode only)_ — probes `macro-ahk-v51`, `…-v25`, … in parallel for a higher-numbered repo. Off by default. Run with `--enable-sibling-discovery` or `SIBLING_DISCOVERY_ENABLED=1`.
