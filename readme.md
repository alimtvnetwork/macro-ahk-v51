...
```powershell
$env:MARCO_DL_VERSION='v3.104.2'; $env:MARCO_DL_FOLDER='marco'; irm https://raw.githubusercontent.com/aukgit/macro-ahk-v51/main/scripts/download-extension.ps1 | iex
```

**Windows · PowerShell — run a local clone with explicit flags:**

```powershell
.\scripts\download-extension.ps1 -Version v3.104.2 -FolderName marco-extension
```

After it finishes, load the resulting folder via `chrome://extensions → Load unpacked`. The `marco-extension-v3.104.2.zip` backup sits next to it.
...
### 📌 Pinned version (`v3.104.2`)

```powershell
# Windows · PowerShell
irm https://github.com/aukgit/macro-ahk-v51/releases/download/v3.104.2/install.ps1 | iex
...
```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/aukgit/macro-ahk-v51/releases/download/v3.104.2/install.sh | bash


```

**Pinned version:** `v3.104.2` &nbsp;•&nbsp; **Macro Controller:** `v3.104.2`
...
- 🔒 Pinned to `v3.104.2` — see [Pinned to a specific release](#-pinned-to-a-specific-release) to change versions
...
- 🔒 Pinned to `v3.104.2` — see [Pinned to a specific release](#-pinned-to-a-specific-release) for other tags
...
```powershell
# PowerShell (Windows) — replace v3.104.2 with any released tag
irm https://github.com/aukgit/macro-ahk-v51/releases/download/v3.104.2/install.ps1 | iex
```
...
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/aukgit/macro-ahk-v51/main/scripts/install.ps1))) -InstallDir (Join-Path $PWD 'marco-extension')
```

```bash
# Bash (Linux / macOS) — installs to ./marco-extension under the current folder
curl -fsSL https://raw.githubusercontent.com/aukgit/macro-ahk-v51/main/scripts/install.sh | bash -s -- --dir "$PWD/marco-extension"
```
...
```powershell
& { $Version = "v2.220.0"; irm https://raw.githubusercontent.com/aukgit/macro-ahk-v51/main/scripts/install.ps1 | iex }
```

```bash
curl -fsSL https://raw.githubusercontent.com/aukgit/macro-ahk-v51/main/scripts/install.sh | bash -s -- --version v2.220.0
```
...
|------|-------------|---------|
| `-Version` | Pin a specific release (`vX.Y.Z[-pre]`) or `latest` | `-Version v3.104.2` |
| `-InstallDir` | Custom install directory | `-InstallDir D:\marco-extension\v3.6.1` |
| `-Repo` | Override GitHub `owner/repo` | `-Repo aukgit/macro-ahk-v51` |
...
|------|-------|-------------|---------|
| `--version <ver>` | `-v` | Force a specific version (`vX.Y.Z[-pre]`) or `latest`. Anything else exits 3. | `--version v3.104.2` |
| `--dir <path>` | `-d` | Target directory (default: `~/marco-extension`) | `--dir ~/marco-extension/v3.104.2` |
| `--repo <o/r>` | `-r` | Override GitHub `owner/repo` | `--repo aukgit/macro-ahk-v51` |
| `--dry-run` | — | Resolve the install plan, print it, exit 0 — **nothing is downloaded or extracted**. Useful for CI smoke tests and debugging. | `--dry-run --version v3.104.2` |
...
./install.sh --version v3.104.2 --dir ~/marco-extension/v3.104.2
...
1. Download `marco-extension-v{VERSION}.zip` from [Releases](https://github.com/aukgit/macro-ahk-v51/releases)

2. Extract to a folder (e.g., `D:\marco-extension\v3.104.2`)
...
Creating or pushing a `release/*` branch (e.g., `release/v3.104.2`) or creating a
...
**No email or notification is sent** — check the [Releases page](https://github.com/aukgit/macro-ahk-v51/releases) for status.
...
```powershell
irm https://github.com/aukgit/macro-ahk-v51/releases/download/v{VER}/install.ps1 | iex
```

**Linux / macOS**

```bash
curl -fsSL https://github.com/aukgit/macro-ahk-v51/releases/download/v{VER}/install.sh | bash
```
