...
function _showStartupResumeDialog(count: number): void {
  const cPanelBg = state.cPanelBg || '#1a1625';
  const overlay = document.createElement('div');
  overlay.style.cssText = 'position:fixed;top:20px;right:20px;width:300px;background:' + cPanelBg + ';border:2px solid ' + (state.cPrimary || '#7c3aed') + ';border-radius:12px;z-index:2147483647;box-shadow:0 10px 40px rgba(0,0,0,0.5);padding:16px;display:flex;flex-direction:column;gap:12px;animation:slide-in-right 0.3s ease-out;';
  
  const title = document.createElement('div');
  title.style.cssText = 'font-size:14px;font-weight:700;color:' + (state.cPrimaryLight || '#a78bfa') + ';display:flex;align-items:center;gap:8px;';
  ...
  ignoreBtn.style.cssText = 'padding:6px 12px;font-size:11px;background:rgba(255,255,255,0.05);border:1px solid ' + (state.cPanelBorder || '#2d2b3b') + ';border-radius:6px;color:#9ca3af;cursor:pointer;';
  ...
  resumeBtn.style.cssText = 'padding:6px 12px;font-size:11px;background:' + (state.cPrimary || '#7c3aed') + ';border:none;border-radius:6px;color:#fff;font-weight:600;cursor:pointer;';
