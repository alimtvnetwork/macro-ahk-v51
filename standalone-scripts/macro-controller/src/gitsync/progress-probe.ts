...
export async function probeProgress(
    ...
    if (!wsId || !projectId || !jobId) {
        throw new Error('probeProgress: missing wsId/projectId/jobId');
    }
    const sdk = getSdk();
    if (!sdk) {
        const msg = 'marco.api.call unavailable (SDK not injected)';
        logError('GitsyncProbe', 'probeProgress: ' + msg
